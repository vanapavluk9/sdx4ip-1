...
proot-distro login debian --shared-tmp -- /bin/bash -c 'apt update && apt upgrade && apt install xfce4 xfce4-goodies sudo nano chromium git -y && echo "You need to setup password, but remember your password (if you dont enter password you cant install apps)" && sleep 3 && adduser debianuser && echo "chromium --no-sandbox" > /usr/bin/chromium. && rm /etc/sudoers && git clone https://github.com/vanapavluk9/sdx4ip-1.git && mv sdx4ip-1/sudoers /etc/ && chmod +x /etc/sudoers && rm -r sdx4ip-1 && exit'
